...
import { Check, ArrowRight, ArrowLeft, Terminal, ShieldAlert, X, Send, AlertTriangle } from "lucide-react"
...
export function RegistrationForm() {
  ...
  const validateStep2 = (): boolean => {
    ...
    if (!formData.guardianPhone.trim()) e.guardianPhone = "Guardian's phone number is required"
    else if (!/^\+?[0-9\s-]{9,15}$/.test(formData.guardianPhone)) e.guardianPhone = "Enter a valid phone number"
    if (!formData.email.trim()) e.email = "Email address is required"
    else if (!/\S+@\S+\.\S+/.test(formData.email)) e.email = "Enter a valid email address"
    if (!formData.inspiration.trim()) e.inspiration = "Please answer this question"
    else if (formData.inspiration.length > 500) e.inspiration = "Must be 500 characters or fewer"
              ...
              {step === 2 && (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <FloatingInput id="phone" label="Student Phone Number" value={formData.phone} onChange={(v) => updateField("phone", v)} error={errors.phone} />
                  </div>

                  {/* Guardian Phone + Email */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <FloatingInput id="guardianPhone" label="Guardian's Phone Number" value={formData.guardianPhone} onChange={(v) => updateField("guardianPhone", v)} error={errors.guardianPhone} />
                    <FloatingInput id="email" label="Email Address" type="email" value={formData.email} onChange={(v) => updateField("email", v)} error={errors.email} />
                  </div>

                  {/* Inspiration Writing Section ("Tell Us About You") */}
                  <div className="space-y-3 pt-4 border-t border-border/20">
                    <div className="flex justify-between items-center">
                      <label htmlFor="inspiration" className="block font-mono text-[10px] uppercase tracking-widest text-foreground/80 font-bold">
                        Tell Us About You <span className="text-accent">*</span>
                      </label>
                      <span className={cn("font-mono text-[10px]", formData.inspiration.length > 500 ? "text-accent font-bold" : "text-foreground/40")}>
                        {formData.inspiration.length}/500
                      </span>
                    </div>
                    <p className="font-mono text-xs text-foreground/60 leading-relaxed">
                      What inspires you to learn programming, and what do you hope to get out of this bootcamp?
                    </p>
                    <textarea
                      id="inspiration"
                      rows={4}
                      ...
                      placeholder="Share your motivation and goals here (max 500 characters)..."
                      className={cn(
                        "w-full bg-background/50 border border-foreground/30 p-3.5 font-mono text-sm text-foreground placeholder:text-foreground/30",
                        "focus:border-accent focus:outline-none transition-colors duration-300 resize-none leading-relaxed",
                        errors.inspiration && "border-accent/70"
                      )}
                    />
                    {errors.inspiration && <p className="font-mono text-[11px] text-accent font-medium tracking-wider">{errors.inspiration}</p>}
                  </div>
        ...
        {isSubmitted && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6 bg-black/80 backdrop-blur-md overflow-y-auto">
            <motion.div
              ...
              className="relative w-full max-w-lg border border-accent/50 bg-card/95 p-6 md:p-8 space-y-6 shadow-[0_0_50px_rgba(230,120,30,0.18)]"
            >
              {/* Corner frame decoration */}
              <div className="absolute -top-px left-0 right-0 h-px bg-gradient-to-r from-accent via-accent/60 to-transparent" />
              ...
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="font-mono text-[10px] uppercase tracking-widest text-accent">SUBMISSION SUCCESSFUL</span>
                    <h3 className="font-[var(--font-bebas)] text-3xl md:text-4xl tracking-wide text-foreground leading-none">
                      THANK YOU!
                    </h3>
                  </div>
                </div>
                <button
                  ...
                  className="p-2 border border-border/40 hover:border-accent text-foreground/60 hover:text-accent transition-colors duration-200 cursor-pointer"
              ...
              <p className="font-mono text-xs text-foreground/80 leading-relaxed">
                Thank you, <span className="text-accent font-semibold">{formData.name}</span>. Your application for the <span className="text-foreground">{bootcampConfig.title}</span> has been logged.
              </p>

              {/* Highlighted Telegram verification box */}
              <div className="border border-accent/60 bg-accent/10 p-4 space-y-2 relative overflow-hidden">
                <div className="flex items-center gap-2 font-mono text-xs uppercase tracking-widest text-accent font-bold">
                  <Send className="w-4 h-4" />
                  <span>Telegram Contact Notice</span>
                </div>
                <p className="font-mono text-xs text-foreground/90 leading-relaxed">
                  If selected, we will reach out directly via your Telegram username:
                </p>
                <div className="py-1 px-3 bg-black/60 border border-accent/40 inline-block font-mono text-sm md:text-base text-accent font-bold tracking-wider">
                  {formData.telegram}
                </div>
                <p className="font-mono text-[11px] text-accent/90 italic pt-1 leading-snug">
                  Please double-check that your Telegram username above is accurate before closing this window!
                </p>
              </div>

              {/* Summary table */}
              <div className="bg-background/80 border border-border/40 p-4 font-mono text-[11px] text-muted-foreground space-y-1.5">
                <div><span className="text-accent">&gt;</span> STATUS: <span className="text-foreground">pending_review</span></div>
                <div><span className="text-accent">&gt;</span> SCHOOL: <span className="text-foreground">{formData.school} ({formData.grade})</span></div>
                <div><span className="text-accent">&gt;</span> STUDENT PHONE: <span className="text-foreground">{formData.phone}</span></div>
                <div><span className="text-accent">&gt;</span> GUARDIAN PHONE: <span className="text-foreground">{formData.guardianPhone}</span></div>
              </div>
              ...
              <div className="pt-2 flex justify-end">
                <button
                  onClick={() => { setFormData(initialData); setStep(1); setIsSubmitted(false) }}
                  className="w-full sm:w-auto bg-accent text-black font-mono text-xs uppercase tracking-widest font-bold px-6 py-3 border border-accent hover:bg-transparent hover:text-accent transition-all duration-200 cursor-pointer text-center"
                >
                  I Checked &amp; Confirmed — Close
                </button>
              </div>
            </motion.div>
          </div>
